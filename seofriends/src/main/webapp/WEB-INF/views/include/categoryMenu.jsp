...
<c:forEach items="${mainCategoryList }" var="categoryVO" > 
  <!-- 1차 카테고리 -->
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" id="subCategory"></div>
      </li>
    </c:forEach>
     <!--  
      <li class="nav-item">
...
$(function () {
    ...
    /* result : 2차 카테고리 정보  */
     $.getJSON(url, function (result) {
      ...
      let subCategoryStr = "";
        ...
        // 1) 직접 주소 작업.
        subCategoryStr += "<a class='dropdown-item' href='/user/product/productList/" + result[i].catecode + "'>" + result[i].catename + "</a>";
        // 2) JQuery 문법을 사용하여, 이벤트 적용을 통한 주소 요청 작업 
        // subCategoryStr += "<a class='dropdown-item' href='" + result[i].catecode + "'>" + result[i].catrname + "</a>";
  ...
  // 2차 카테고리 클릭.
/*   $("ul.navbar-nav li.nav-item div.subCategoryStr").on("click", "a", function(e) {
    e.preventDefault(); //<a href=""></a> 링크 기능을 제거한다.

    let catecode = $(this).attr("href");
    location.href = "/user/product/productList/" + catecode;
