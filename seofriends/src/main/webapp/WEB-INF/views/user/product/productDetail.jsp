...
<div class="pricing-header px-3 py-3 pt-md-5 pb-md-4 mx-auto text-center">
  <h1 class="display-4">${catename}</h1>
  ...
  <script>
    ...
    $(function(){
      ...
      $("div#productDetailReview button#btnReview").on("click", function(){
        ...
        $("button#btnReviewWrite").show();  // 상품후기 쓰기 버튼만 보이게 함.
      ...
      $("#btnReviewModify").on("click", function(){
       ...
       $("div#reviewListResult").on("click", "p a.delete", function(e){
        ...
        if (!confirm( rv_num + "번 상품후기를 삭제하시겠습니까?")) return;
        //자바스트립트 Object객체 구문
        //let data = { rv_num : rv_num };
        $.ajax({
          url: '/user/review/delete/' + rv_num,
          headers: {
    ...
    // //로그인사용자와댓글작성자가 일치할 경우 쓰기버튼 표시
    // Handlebars.registerHelper("writeview", function(review_mem_id, rv_num) {

    // let result = "";
    // let login_mem_id = "${sessionScope.loginStatus.mem_id}";
    // if (review_mem_id == login_mem_id) {
    //   result = "<a class='write' href='" + rv_num + "'><b>[댓글쓰기]</b></a>";
    // }
    ...
    //로그인사용자와 댓글작성자가 일치할 경우 수정버튼 표시
    Handlebars.registerHelper("modifyview", function(review_mem_id, rv_num) {

      let result = "";
      let login_mem_id = "${sessionScope.loginStatus.mem_id}";
      if (review_mem_id == login_mem_id) {
        result = "<a class='modify' href='" + rv_num + "'><b>[댓글수정]</b></a>";
      }
    ...
    let login_mem_id = "${sessionScope.loginStatus.mem_id}";
    if (review_mem_id == login_mem_id) {
      result = "<a class='delete' href='" + rv_num + "'><b>[댓글삭제]</b></a>";
    }
